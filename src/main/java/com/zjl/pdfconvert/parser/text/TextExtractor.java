...
public class TextExtractor extends CustomLegacyPDFStreamEngine {
    ...
    public void processText(TextPosition text) {
        ...
        style.setAbx((int) text.getEndX());
        style.setAby((int) text.getEndY());
    ...
    public void processLine(List<TextExtractor.WordWithTextPositions> line) {
        ...
        ls.setWord(firstWord);
        while (cursor > 1) {
            if (!this.line.isEmpty()) {
                Fact word = this.line.pollFirst();
                if (word instanceof Word) {
                    ((Word) word).setLineIndex(lineIndex);
                }
                this.words.add(word);
            }
            cursor--;
